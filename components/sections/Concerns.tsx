        ...
        <ul className="mx-auto mt-10 grid max-w-4xl gap-3 sm:grid-cols-2">
          {concerns.map((c) => (
            <li key={c.href} className="reveal">
              <Link
                href={c.href}
                className="group flex min-h-16 items-center gap-3 rounded-2xl border border-line bg-white px-4 py-3 text-base font-medium text-ink shadow-card transition-colors hover:border-brand-300 hover:bg-brand-50 sm:text-lg"
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 text-brand-700">
                  <CheckIcon size={18} />
                </span>
                <span className="flex-1">{c.text}</span>
                <ArrowIcon size={18} className="text-brand-600 transition-transform group-hover:translate-x-0.5" />
              </Link>
            </li>
          ))}
        </ul>
